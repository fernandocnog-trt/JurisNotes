...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (window.PdfEngine) {
        PdfEngine.init({
            ...
            onProcessoIdentificado: (numeroCurto) => {
                ...
                window._nomeArquivoSugerido = `AI_${numeroCurto}.json`;
            },
    ...
    if (window.ShortcutManager) window.ShortcutManager.init(); // <-- INICIALIZA O ICON REGISTRY DOS FABS NO AI
...
window.atualizarStatusBotaoExtrator = function() {
    ...
    if (hasSentencaInicio && hasSentencaFim && hasRecursoInicio && hasRecursoFim) {
        ...
        btn.title = 'Contexto Completo! (Decisão e Recurso mapeados)';
    } else {
        btn.classList.remove('extrator-completo');
...
window.TaskManager = (function() {
    ...
    // Exporta estado das tarefas para persistência
