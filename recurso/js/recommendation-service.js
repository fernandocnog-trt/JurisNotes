...
${catalogoComprimido}`;
        ...
        for (let i = 0; i < GROQ_MODELS.length; i++) {
            ...
            if (i > 0) {
                ...
                // Pequena pausa estratégica de 1.5s para não ativar proteção contra DDoS da Groq
            ...
            try {
                ...
                if (!response.ok) {
                    // Erro 401 ou 403 é fatal (Chave errada/expirada). Não fazemos fallback.
                    if (response.status === 401 || response.status === 403) {
                        localStorage.removeItem(STORAGE_KEY);
                        throw new Error("Sua API Key do Groq é inválida. A chave foi limpa.");
                    }
                    // Outros erros (429 Cota, 503 Fora do Ar), disparam o erro para ir ao próximo modelo
                ...
                // Sucesso absoluto! Interrompe o loop (não tenta os próximos modelos)
                break; 
        ...
        // CONTINUAÇÃO DA FUNÇÃO ORIGINAL: PIPELINE DE SANITIZAÇÃO ESTRUTURAL BLINDADO
        try {
